	...
	write() {
		...
		this.update_blocks(produce((blocks) => {
			...
			blocks.forEach((block) => {
				let length = block.output?.length
				block.start = start
				start += length
			})
		}))
...
function code_element(state, index, control) {
	...
	let extensions = [EditorView.updateListener.of(throttle((e) => live_code.set(e.state.doc.toString() + "\n"), 100))]

	const render = () => {
		mounted(() => {
			const editor = make_code_mirror(code(), id, extensions);
			focus = () => {
				console.log("called");
