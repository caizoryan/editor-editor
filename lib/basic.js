...
class State {
	...
	overwrite(path) {
		...
		const body = JSON.stringify(this.model, null, 2);
	...
	create_new(path) {
		...
		fetch("/fs/" + path, {
			headers: { "Content-Type": "application/json" },
			method: "POST",
			body: JSON.stringify(this.model, null, 2),
		});
	}
...
function RootRenderer(el, state) {
	...
	keys.on("shift+m", (_) => state.save(save_path));
	keys.on("shift+l", (_) => state.load(save_path));
...
let f = "/";
let t1 =
	'<script type="module"> import {mut} from "/lib/solid/monke.js"; let M = mut({}); document.M = M;';
let t2 = "<" + f + "script>";

const iframe = mem(() =>
	t1 + editor.state.blocks.map((e) => e.output).join("\n") + t2
);
...
const Root = () => {
	...
	const editor_style = editor_pos.css;
