...
class State {
	...
	write() {
		...
		this.model.blocks.forEach((b, i) => console.log("block " + i + " starts at: ", b.start))
	...
	async lint(from, to) {
		if (!this.tsserver) {
			...
			return this.parent.lint(from + this.start, to + this.start)
		}
		console.log("from", from, "to", to)
		const diagnostics = this.tsserver.semantic_diagnostics();
		...
		return diagnostics
			...
			.map(d => {
				...
				if (f < 0) {
					console.log("f less than 0", f, "from", from, "d.start", d.start)
					return
				}

				if (d.start + d.length > to) {
					console.log("beyond to", d.start + d.lenght, "to", to)
					return
				}

				let message = d.messageText
				console.log("f", f, "t", t, "message", message)
	...
	async completion(pos, ctx) {
		...
		if (!this.tsserver) {
			// then send req to parent and add personal start
			return this.parent.completion(pos + this.start, ctx)
		}

		console.log("pos", pos)

		const completions = this.tsserver.completion_at(pos);
...
class Tsserver {
	...
	update_file(content) {
		...
		if (this.env) { this.env.updateFile("index.ts", content) }
		else this.file = content
	}

	completion_at(pos) {
		if (this.env) return this.env.languageService.getCompletionsAtPosition('index.ts', pos)
		else return []
	}

	semantic_diagnostics() {
		if (this.env) return this.env.languageService.getSemanticDiagnostics('index.ts')
		else return []
	}

	syntactic_diagnostics() {
		if (this.env) return this.env.languageService.getSyntacticDiagnostics('index.ts')
		else return []
	}
...
class Editor {
	constructor({ state, components, renderer }) {
		...
		eff_on(this.output, () => {
			this.tsserver.update_file(this.output())
			console.log("output", this.output())
			// let syntax = this.tsserver.syntactic_diagnostics()
			// let semantics = this.tsserver.semantic_diagnostics()
			// console.log("syntax", syntax, "semantics", semantics)
		})

		eff_on(this.live_output, () => {
			console.log("live output changed", this.live_output())
...
function state_utils(state) {
	...
	const unfocus_current = () => {
		...
		console.log("unfocussing current: ", current)
		if (!current) return;
...
eff_on(iframe, () => defer(function() { m().EDITOR = editor }, 100))
...
function code_element(state, index, control) {
	...
	eff_on(live_output, () => {
		console.log("should change,,,,", live_output())
			...
			let extensions = [
				...
				autocompletion({
					...
					override: [async (ctx) => {
						...
						return control.state.completion(pos + start, ctx)
					}],
				}),
