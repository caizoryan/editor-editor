...
const RootRenderer = (el, state) => {
	...
	keys.add("Enter", set_current_focus);
	keys.add("shift+c", (_) => add_widget({ type: "code" }));
	keys.add("shift+a", (_) => add_widget({ type: "basic" }));
	keys.add("shift+s", (_) => state.write());
	keys.add("j", (_) => state.next());
	keys.add("k", (_) => state.prev());
