...
const RootGroup = (el, i, c) => {
	...
	const bind = (child, index) => {
		let setter = (...args) => state.update("blocks", index(), ...args);
		let controller = { set_self: setter };

		const render = renderers.find(child.type);
		const component = render(child, i, control);
