...
class State {
	...
	write() {
		...
		console.log(this.model.blocks.map((e) => e.output).join("\n"));
	}
}
...
const RootRenderer = (el, state) => {
	...
	const keymanager = (e) => {
		switch (e.key) {
			...
			case "C":
				add_widget({ type: "code" });
				break;

			case "G":
				add_widget({ type: "RootGroup" });
				break;
...
editor.renderers.register("basic", basic.toString());
editor.renderers.register("RootGroup", RootRenderer.toString());
editor.renderers.register("code", code_element.toString());
