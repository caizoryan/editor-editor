...
const m = () => {
	console.log("ref", iframe_ref())
	return iframe_ref()?.contentDocument.M
}
...
class State {
	...
	async lint(from, to) {
		...
		const sem_diagnostics = await this.tsserver.semantic_diagnostics();
		const syn_diagnostics = await this.tsserver.syntactic_diagnostics();
