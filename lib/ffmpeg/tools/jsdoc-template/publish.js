...
/*global env: true */
'use strict';
var taffy = taffydb.taffy, htmlsafe = helper.htmlsafe, linkto = helper.linkto, resolveAuthorLinks = helper.resolveAuthorLinks, scopeToPunc = helper.scopeToPunc, hasOwnProp = Object.prototype.hasOwnProperty, data, view, outdir = env.opts.destination;
function find(spec) {
...
function buildNav(readmeNav, members) {
    var nav = '<h2><a href="index.html">Index</a></h2>' + readmeNav, seen = {}, hasClassList = false, classNav = '', globalNav = '';
...
export const publish = function (taffyData, opts, tutorials) {
    data = taffyData;
    ...
    var files = find({ kind: 'file' }), packages = find({ kind: 'package' });
    generate('Index', packages.concat([{ kind: 'mainpage', readme: opts.readme, longname: (opts.mainpagetitle) ? opts.mainpagetitle : 'Main Page' }]).concat(files), indexUrl);
    ...
    function generateTutorial(title, tutorial, filename) {
        ...
        var tutorialPath = path.join(outdir, filename), html = view.render('tutorial.tmpl', tutorialData);
